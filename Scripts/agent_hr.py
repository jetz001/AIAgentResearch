...
# ── Roleplay UI Helpers ──
def print_box(title, content, color_code="36"): # Default Cyan for HR
...
def generate_report(work_details: str):
    ...
    report_content = llm_helper.get_report_response("HR Agent", work_details, my_role, agent_key="hr")
    
    print_box("📄 REPORT: 👥 HR Agent", report_content, "36")
    
    # บันทึกลงไฟล์
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(PROJECT_ROOT, "Output", "reports")
    os.makedirs(output_dir, exist_ok=True)
    report_file = os.path.join(output_dir, f"hr_report_{timestamp}.md")
...
def log_action(action, phase="IMPLEMENTATION"):
    with open(os.path.join(LOGS_DIR,"hr_agent.log"),"a",encoding="utf-8") as f:
        f.write(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] [HR] [{phase}] {action}\n")
...
def main(initial_message=""):
    ...
    if initial_message and initial_message != "(ไม่มีข้อความ)":
        ...
        roleplay_msg = llm_helper.get_roleplay_response("HR Agent", initial_message, my_role, agent_key="hr")
        
        print_box("🎭 ROLEPLAY: 👥 HR Agent", roleplay_msg, "36")
