...
def generate_report(work_details: str):
    ...
    report_content = llm_helper.get_report_response("Editor Agent", work_details, my_role, agent_key="editor")
    
    print_box("📄 REPORT: 📰 Editor Agent", report_content, "32")
    
    # บันทึกลงไฟล์
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(PROJECT_ROOT, "Output", "reports")
    os.makedirs(output_dir, exist_ok=True)
    report_file = os.path.join(output_dir, f"editor_report_{timestamp}.md")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(f"# 📊 Editor Report\n")
...
def log_action(action, phase="IMPLEMENTATION"):
    with open(os.path.join(LOGS_DIR,"editor_agent.log"),"a",encoding="utf-8") as f:
        f.write(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] [Editor] [{phase}] {action}\n")
